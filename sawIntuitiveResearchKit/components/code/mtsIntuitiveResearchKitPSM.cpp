    ...
    case mtsIntuitiveResearchKitArmTypes::DVRK_READY:
        {
            ...
            // first two rotations
            tolerances.Ref(2, 0).SetAll(170.0 * cmnPI_180);
            // translation
            tolerances.Element(2) = 170.0 * cmn_mm; // 10 mm
            // shaft rotation
            tolerances.Element(3) = 170.0 * cmnPI_180;
            // tool orientation
            tolerances.Ref(4, 3).SetAll(175.0 * cmnPI_180);
            // gripper
...
void mtsIntuitiveResearchKitPSM::RunEngagingAdapter(void)
{
    ...
    if (EngagingStage == 1) {
        ...
        tolerances.Ref(2, 0).SetAll(170.0 * cmnPI_180); // 10 degrees
        tolerances.Element(2) = 170.0 * cmn_mm; // 10 mm
